...
class Solution:
    def reconstructQueue(self, people: [[int]]) -> [[int]]:
        ...
        我们就是要模拟一个插入队列的行为，所以不应该使用数组，而是要使用链表！但python貌似不能这么做这题。
        ...
        def cmp(item1, item2):
            if item1[0] == item2[0]:
                if item1[1] < item2[1]:     # position更小的排在前面，所以return -1，表示less than
                    return -1
                else:
                    return 1
            else:
                if item1[0] < item2[0]:     # 因为是升序排列，所以return 1，值更小的反而表示greater than
                    return 1
                else:
                    return -1
