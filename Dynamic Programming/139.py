...
class Solution1:
    ...
    def wordBreak(self, s: str, wordDict: List[str]) -> bool:
        ...
        Time O(2^n * n)
        Space O(n)
        时间复杂度分析，对于长度为n的s，总共有n-1种切缝，每个字符有两种情况，切和不切，所以总共有2^n个节点，
        每个递归函数里面需要一个O(n)的loop，check节点存在为O(1)，所以总共为O(2^n * n * 1)。
...
class Solution2:
    ...
    def wordBreak(self, s: str, wordDict: List[str]) -> bool:
        ...
        Time O(n * n)
        Space O(n)
        时间复杂度分析，对于长度为n的s，总共有n-1种切缝，每个字符有两种情况，切和不切，所以总共有2^n个节点，但是我们用了备忘录避免了重复的
        切割，所以每个切缝只会切一次，也就是O(n)，每此递归函数的调用是O(n * 1)同上。
