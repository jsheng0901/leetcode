class Solution:
    ...
    def backtracking(self, s, start_index, l, r):
        if l == 0 and r == 0 and self.is_valid(s):      # check if valid
            self.result.append(s)

        for i in range(start_index, len(s)):
            if i - 1 >= start_index and s[i] == s[i - 1]:       # jump repeat
                continue
            if r > 0 and s[i] == ')':                           # 删除右括号
                self.backtracking(s[:i] + s[i + 1:], i, l, r - 1)
            if l > 0 and s[i] == '(':                           # 删除左括号
                self.backtracking(s[:i] + s[i + 1:], i, l - 1, r)
